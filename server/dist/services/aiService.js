...
export async function getRecommendations(userId) {
    ...
    if (env.featureAi && insights.length) {
        await supabaseAdmin.from("ai_insights").insert(insights.map((ins) => ({
            scope: ins.scope,
...
export async function getEventInsights(eventId) {
    ...
    const sentimentScore = (chatCount ?? 0) > 20 ? 0.72 : (chatCount ?? 0) > 5 ? 0.55 : 0.4;
    return [
        {
            id: `forecast-${eventId}`,
            scope: "event",
            scope_id: eventId,
            insight_type: "attendance_forecast",
            payload: {
